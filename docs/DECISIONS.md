...
| 66 | 8C | `ApiError` carries the body's `error` code; `isLlmUnavailable()` is true only for code `llm_unavailable`. The channel page (Explain) and the chat drawer show a calm grey inline notice with Retry for it; every other error keeps the existing red message. | Only the "AI is down" case is expected and harmless; real errors stay visible. |
| 67 | 8C | Retry re-sends exactly the failed request (same channel; same chat history). A successful retry removes the notice. | No duplicate user turns in chat. |
| 68 | 8C | Agent answers show `answered by <provider>` in small grey text when the backend reports a provider. | Transparency about which model wrote the text. |
| 69 | 8C | Test setup stubs `ResizeObserver` (jsdom lacks it) so full pages with recharts can be rendered in vitest. | Needed to test the channel page itself. |
| 70 | 8D | Cloudflare chat uses `CloudflareChatOpenAI`, a `ChatOpenAI` subclass that rewrites the request only: list content -> text parts joined by newlines (non-text parts such as thinking dropped), `null` -> `""`; tool calls, tool role and `tool_call_id` untouched. OpenAI, Gemini and Anthropic keep their stock classes. (F1, F2) | Workers AI rejects non-string content (live 400). |
| 71 | 8D | Tool-call `arguments` returned as a JSON object are re-encoded as a string before LangChain parses the response. (F1) | Workers AI's native format; harmless if `/ai/v1` already returns strings. |
| 72 | 8D | Recommended chain `LLM_FALLBACK_PROVIDERS=cloudflare,gemini` is documented (README, `.env.example`); the code default stays empty so local dev uses `LLM_PROVIDER` (Ollama). (F3) | Gemini free tier ~20 RPD; no billing. |
| 73 | 8E | `EMBEDDING_PROVIDER=cloudflare` uses `OpenAIEmbeddings` pointed at CF `/ai/v1` with `@cf/baai/bge-m3` (`CF_EMBEDDING_MODEL`), `check_embedding_ctx_length=False` (raw strings, no tiktoken token ids) and `max_retries=0` (the existing `RetryingEmbeddings` retries: 1s, 3s; no fallback). | Same endpoint family as chat; no new dependency. |
| 74 | 8E | `sync_index` deletes vectors of every other embedding model before indexing, then embeds only missing chunks. A provider switch re-indexes once; repeated syncs embed nothing. Switching back re-embeds again. | "Drop old vectors, embed again, idempotent"; never mixes dimensions. |
| 75 | 8E | The old test asserting "cloudflare is not an embedding provider" now asserts that an unknown provider name is rejected. | Behaviour intentionally changed in 8E. |
| 76 | 8F | Agent tools resolve `channel` through one resolver: case, surrounding spaces, spaces/hyphens/underscores are ignored, and the display name is accepted ("Google Search" -> `google_search`). No prefix or fuzzy matching ("goog", "met" are rejected). Unknown names return `Channel '<x>' not recognized. Valid channel ids: …`. | Llama sent `channel="Meta"`; exact names only avoid guessing the wrong channel. |
| 77 | 8F | Tools that take a channel list the valid ids (read from the `channels` table) in their description when bound for chat. | The model sees the ids it may use. |
