...
| P10 | 2026-09-25 | **Resolved (live evidence, Railway, chain `gemini,cloudflare`):** `/agent/chat` on Cloudflare `/ai/v1` did not give usable tool calls in our path; the chain fell back to Gemini (`provider:"gemini"`, grounded). `/agent/explain` on Cloudflare worked (`provider:"cloudflare"`, grounded, all numbers cited). In practice Cloudflare is an explain-only fallback. | Chat depends on Gemini being up; if it is down, chat gets `llm_unavailable`. |
