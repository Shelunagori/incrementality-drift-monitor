...
| P10 | 2026-09-25 | **Reopened (8D).** Root cause of `/agent/chat` failing on Cloudflare: Workers AI `/ai/v1` requires every message `content` to be a string; LangChain sent `null` on assistant tool-call messages and a list of parts for list content (seen after a Gemini turn). Live 400: `Type mismatch of '/messages/0/content', 'array' not in 'string'; '/messages/2/content', 'string' not in 'null'`. Earlier evidence: explain on CF worked (no tool messages reach the model); chat fell back to Gemini. **Fix in 8D:** Cloudflare-only request normalisation + object-`arguments` parsing, tested against a synthetic CF-shaped response. **Close only after the live check on Railway.** | Until verified live, chat still depends on Gemini, whose free tier is ~20 requests/day. |
...
| P12 | 2026-09-25 | Embeddings are Gemini-only (`gemini-embedding-001`). Google no longer publishes free-tier numbers in the docs (AI Studio only; last published ~100 RPM / 1,000 RPD — verify). Risk to `search_methodology` is low: chunks are embedded once and stored, then ~1 query embedding per explain; on quota errors the tool returns an error and the explanation continues (8A). **Planned as phase 8E:** Cloudflare `@cf/baai/bge-m3` (1024-dim) via CF `/v1/embeddings`, `EMBEDDING_PROVIDER=cloudflare`, re-index `knowledge_chunks` when the stored model id differs (no migration needed: the vector column has no fixed dimension and rows carry the model id). Effort ~0.5–1 day incl. tests. | Removes the last hard dependency on the Gemini free tier. |
