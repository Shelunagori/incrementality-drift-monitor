...
| P12 | 2026-09-25 | **Implemented in 8E, live check pending.** Embeddings were Gemini-only (`gemini-embedding-001`; free-tier numbers only in AI Studio, last published ~100 RPM / 1,000 RPD). 8E adds `EMBEDDING_PROVIDER=cloudflare` (`@cf/baai/bge-m3`, 1024-dim, CF `/ai/v1/embeddings`, same retries, no fallback) and drops vectors of any other model on the next index sync, so a switch re-indexes once. Tested against a synthetic CF response, not a recorded one. Close after the first explain on Railway with `EMBEDDING_PROVIDER=cloudflare` shows `search_methodology` results. | Removes the last hard dependency on the Gemini free tier. |
